"""Audit action mail_data_purged + permisos de purga local de correo.

Revision ID: 0007_mail_data_purge_rbac
Revises: 0006_mailbox_delegation
"""
...
revision: str = "0007_mail_data_purge_rbac"
down_revision: Union[str, None] = "0006_mailbox_delegation"
...
def upgrade() -> None:
    op.execute(
        sa.text(
            "ALTER TYPE audit_action ADD VALUE IF NOT EXISTS 'mail_data_purged'"
        )
    )
...
def downgrade() -> None:
    # No se eliminan valores de ENUM en Postgres de forma portable.
    pass
